 ...
 */
static BOOL TREEVIEW_AlterItemState(const TREEVIEW_INFO *infoPtr, TREEVIEW_ITEM * item, UINT stateNew)
{
		   ...
		   const TVITEMEXW *tvItem, BOOL isW)
{
    ...
    {
        UINT stateNew = (item->state & ~tvItem->stateMask) | (tvItem->state & tvItem->stateMask);
        TREEVIEW_AlterItemState(infoPtr, item, stateNew);
    }
...
TREEVIEW_ToggleItemState(TREEVIEW_INFO *infoPtr, TREEVIEW_ITEM *item)
{
    if (infoPtr->dwStyle & TVS_CHECKBOXES)
    {
        ...
        if (state != item->state && TREEVIEW_AlterItemState(infoPtr, item, state))
        {
            TREEVIEW_Invalidate(infoPtr, item);
        }
    }
		      ...
		      INT cause)
{
	...
	if (prevSelect == NULL || TREEVIEW_AlterItemState( infoPtr, prevSelect, prevSelect->state & ~TVIS_SELECTED))
    {
        /* deselected previous */
        if (newSelect != NULL) TREEVIEW_AlterItemState( infoPtr, newSelect, newSelect->state | TVIS_SELECTED);
    }
