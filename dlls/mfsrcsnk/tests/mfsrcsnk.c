...
static const IMFByteStreamVtbl teststreamvtbl =
{
...
{
    struct test_stream *stream = calloc(1, sizeof *stream);
    if (!stream)
        return NULL;

    stream->iface.lpVtbl = &teststreamvtbl;
    stream->refcount = 1;
    stream->inner = inner;
...
#undef EMBED_U32
        ...
        _Static_assert(sizeof header == 44);
