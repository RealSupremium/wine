...
static HRESULT next_media_event_(int line, IMFMediaEventGenerator *source, IMFAsyncCallback *callback, DWORD timeout,
        IMFMediaEvent **event)
{
    ...
    ok_(__FILE__, line)(hr == S_OK || hr == MF_S_MULTIPLE_BEGIN, "Unexpected hr %#lx.\n", hr);
        ...
        MediaEventType expect_type, DWORD timeout, PROPVARIANT *value)
{
    ...
    do
    {
        ...
        ret = next_media_event(source, callback, timeout, &event);
...
static void test_sample_times_at_rate(IMFMediaSource *source, FLOAT rate, BOOL thin)
{
    ...
    {
        hr = wait_media_event(source, callback, thin ? MENewStream : MEUpdatedStream, 100, &value);
        ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
    }
    else
    {
        ret = next_media_event(source, callback, 100, &event);
    ...
    hr = wait_media_event(source, callback, MESourceStarted, 100, &value);
    ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
    ok(value.vt == VT_I8, "got vt %u\n", value.vt);
    ...
    ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
    hr = wait_media_event(source, callback, MESourceStopped, 100, &value);
    ok(hr == S_OK, "Unexpected hr %#lx.\n", hr);
