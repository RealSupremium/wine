...
static void test_vbdisp_typeinfo(void)
{
    ...
    /* Use SCRIPTTEXT_ISEXPRESSION to obtain the class instance dispatch directly.
       On native Windows, GetDispID on ScriptDisp returns DISP_E_UNKNOWNNAME for
       variables defined via 'dim' in parsed script text, so we cannot retrieve
       the instance through the script dispatch. */
    SET_EXPECT(OnEnterScript);
    SET_EXPECT(OnLeaveScript);
    ...
    /* GetDocumentation(MEMBERID_NIL) returns the class name */
    hr = ITypeInfo_GetDocumentation(typeinfo, MEMBERID_NIL, &bstr, NULL, NULL, NULL);
    ok(hr == S_OK, "GetDocumentation(MEMBERID_NIL) failed: %08lx\n", hr);
    ok(!lstrcmpW(bstr, L"C"), "Unexpected class name %s\n", wine_dbgstr_w(bstr));
    SysFreeString(bstr);

    /* GetTypeAttr: class C has 4 public funcs (method, add, val, defprop) and 1 public var (x) */
    hr = ITypeInfo_GetTypeAttr(typeinfo, &attr);
    ok(hr == S_OK, "GetTypeAttr failed: %08lx\n", hr);
    ok(attr->typekind == TKIND_DISPATCH, "Unexpected typekind %u\n", attr->typekind);
    ...
    /* Verify all 4 public funcs are enumerable and have correct properties.
       Don't assume ordering - look up by name via GetIDsOfNames instead. */

    /* 'method' - public sub, 0 params, returns void */
    wcscpy(str, L"method");
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ...
    /* 'add' - public function, 2 params, returns variant */
    wcscpy(str, L"add");
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ...
    /* 'defprop' - public default property get */
    ...
    /* GetVarDesc for the public dim 'x' (index 0) */
    ...
    wcscpy(str, L"x");
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ok(hr == S_OK, "GetIDsOfNames(x) failed: %08lx\n", hr);
    wcscpy(str, L"add");
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ok(hr == S_OK, "GetIDsOfNames(add) failed: %08lx\n", hr);
    wcscpy(str, L"val");
    ...
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ok(hr == S_OK, "GetIDsOfNames(METHOD) failed: %08lx\n", hr);
    /* Private members should not be found */
    wcscpy(str, L"strret");
    hr = ITypeInfo_GetIDsOfNames(typeinfo, &names, 1, &memid);
    ...
    /* CreateInstance / AddressOfMember / GetDllEntry always fail */
    obj = (void*)0xdeadbeef;
    hr = ITypeInfo_CreateInstance(typeinfo, NULL, NULL, &obj);
    ok(hr == TYPE_E_BADMODULEKIND, "CreateInstance returned: %08lx\n", hr);
    ok(!obj, "Unexpected non-null obj %p.\n", obj);
