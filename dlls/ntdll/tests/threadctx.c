...
DWORD WINAPI thread_func(LPVOID lpParameter)
{
    ...
    while (looping) {
        ...
        if (status)
            ok(!status, "Failed to get context: 0x%lx\n", (long)status);
        c.ContextFlags = CONTEXT_FLOATING_POINT;
        status = pNtSetContextThread( GetCurrentThread(), &c );
        if (status)
            ok(!status, "Failed to set context: 0x%lx\n", (long)status);
    }
...
#undef X
    ...
    if (!pNtGetContextThread) {
        win_skip("NtGetContextThread not available.\n");
        return;
    }
    if (!pNtSetContextThread) {
        win_skip("NtSetContextThread not available.\n");
        return;
    }
